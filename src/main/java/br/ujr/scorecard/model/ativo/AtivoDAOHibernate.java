...
public class AtivoDAOHibernate extends HibernateDaoSupport implements AtivoDAO {
	...
	public List<Ativo> findByDescricao(String descricao) {
		try {
			StringBuffer strQuery = new StringBuffer(" from Ativo as ativo where upper(ativo.descricao) like ? ");
			List<Ativo> list = this.getHibernateTemplate().find(strQuery.toString(),descricao.toUpperCase());
			return list;
	...
	public List<Ativo> findByReferencia(ContaCorrente contaCorrente, long referenciaInicial, long referenciaFinal) {
		try {
			StringBuffer strQuery = new StringBuffer(" from Ativo as ativo where ativo.referencia >= ? and ativo.referencia <= ? ");
			if ( contaCorrente != null )
				strQuery.append(" and ativo.contaCorrente.id = ? ");
			...
			List<Ativo> list = this.getHibernateTemplate().find(strQuery.toString(),params);
	...
	public List<Ativo> findByReferencia(ContaCorrente contaCorrente, Class ativoClass, long referenciaInicial, long referenciaFinal) {
		try {
			...
			strQuery.append(alias).append(".referencia >= ? and ").append(alias).append(".referencia <= ?");
			if ( contaCorrente != null ) {
				strQuery.append(" and ");
				strQuery.append(alias).append(".contaCorrente.id = ? ");
			}
	...
	public List<Ativo> findByContaContabil(Conta conta) {
		try {
			StringBuffer strQuery = new StringBuffer(" from Ativo as ativo where ativo.conta.id = ?");
			List<Ativo> list = this.getHibernateTemplate().find(strQuery.toString(),new Object[]{conta.getId()});
			return list;
		} catch (DataAccessException e) {
			logger.error(e.getMessage(),e);
			throw e;
