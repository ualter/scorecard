...
public class ProcessadorExtratoCCBancoBrasilGUI extends JDialog implements MouseListener, ActionListener, FocusListener {
	...
	private   int         largura           = 1235;
	private   int         altura            = 602;
	...
	public ProcessadorExtratoCCBancoBrasilGUI() {
        ...
        this.setTitle("Scoredcard - Analisador Extrato Conta Corrente: B A N C O    D O    B R A S I L");
        this.setName("AnalisadorExtratoContaCorrenteBancoBrasil");
        this.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
        
        // Table Nao Encontrados na Base de Dados
        this.modelTabNaoEncontrados = new DefaultModelLinhaExtrato(null,new Object[]{"Data","D.Origem","Histórico","N.o Documento","Valor","Identificação","C.Contabil","Object" });
		...
		JLabel lblLctoNaoEncontrados = new JLabel("Lançamentos  B A N C O   D O   B R A S I L  não encontrados na Base de Dados",SwingConstants.LEFT);
		...
		lblLctoNaoEncontrados.setBackground(Color.YELLOW);
		lblLctoNaoEncontrados.setForeground(new Color(0,0,128));
		...
		this.txtDtRef.setDate(ProcessadorExtratoCCBancoBrasil.foundInThePath());
	...
	/**
	 * CC Santander Alvo transferência
	 * @return
	 */
	...
	private void limparResumo() {
		...
		lblSaldoAnteriorSistema.setForeground(Color.BLUE);
		lblSaldoSistema.setForeground(Color.BLUE);
		this.lblSaldoDiferenca.setText("");
		this.lblSaldoAnteriorDiferenca.setText("");
	}
	...
	private void atualizarDiferenca(BigDecimal saldo, BigDecimal saldoAnterior) {
		...
		if ( resumoSaldo.floatValue() < 0 ) {
			bdSaldoDiferenca = saldo.subtract(this.resumoSaldo.multiply(new BigDecimal(-1)));
		}
	...
	private void carregarTabNaoEncontrados(long ref) {
		ProcessadorExtratoCCBancoBrasil analisador = new ProcessadorExtratoCCBancoBrasil(ref);
		if ( !analisador.checkArquivo() ) {
			JOptionPane.showMessageDialog(this, "Arquivo " + analisador.getPathExtrato() + ", não foi encontrado!", "Atenção",JOptionPane.WARNING_MESSAGE);
			this.txtDtRef.setDate(null);
			((JTextFieldDateEditor)this.txtDtRef.getDateEditor()).requestFocus();
			this.btnRemover.setEnabled(false);
			this.carregarSaldos(new BigDecimal(0), new BigDecimal(0));
			this.limparResumo();
		} else {
			analisador.loadExtrato();
			...
			this.carregarSaldos(analisador.getSaldo().getValorAsBigDecimal(), analisador.getSaldoAnterior().getValorAsBigDecimal());
			List<ProcessadorExtratoCCBancoBrasil.LinhaExtratoContaCorrenteBancoBrasil> listaNaoEncontrados = analisador.getLancamentosNaoExistentesBaseDados();
			for(LinhaExtratoContaCorrenteBancoBrasil linha : listaNaoEncontrados) {
				Object row[] = new Object[] {
						linha.getData() 
						,linha.getDependenciaOrigem()
						,linha.getHistorico()
	...
	private static final int COLUMN_DEPENDENCIA       = 1;
	private static final int COLUMN_HISTORICO         = 2;
	private static final int COLUMN_NUMERO_DOCUMENTO  = 3;
	...
	private void layOutTableNaoEncontrados() {
		...
		TableColumn dependeciaOrigemColumn    = tabNaoEncontrados.getColumnModel().getColumn(COLUMN_DEPENDENCIA);
		...
		DefaultTableCellRenderer dependenciaRenderer     = new AnalisadorExtratoTableCellRenderer();
		...
		dependenciaRenderer.setHorizontalAlignment(DefaultTableCellRenderer.LEFT);
		historicoRenderer.setHorizontalAlignment(DefaultTableCellRenderer.LEFT);
		numeroDocumentoRenderer.setHorizontalAlignment(DefaultTableCellRenderer.LEFT);
		
		dataColumn.setCellRenderer(dataRenderer);
		dependeciaOrigemColumn.setCellRenderer(dependenciaRenderer);
		...
		dependeciaOrigemColumn.setPreferredWidth(67);
		...
		contaColumn.setPreferredWidth(210);
	}
	...
	private class AnalisadorExtratoTableCellRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
			...
			LinhaExtratoContaCorrenteBancoBrasil linha = (LinhaExtratoContaCorrenteBancoBrasil)table.getModel().getValueAt(row, COLUMN_OBJECT);
		...
		private void layOutCellMoney(Object value, LinhaExtratoContaCorrenteBancoBrasil linha) {
			this.layOutCellMoney(value, null, linha);
		}
		private void layOutCellMoney(Object value, Color color, LinhaExtratoContaCorrenteBancoBrasil linha) {
	...
	private class IdentificacaoTableCellEditor extends DefaultCellEditor implements ItemListener, FocusListener {
		...
		public void itemStateChanged(ItemEvent e) {
			...
			if ( row > -1 ) {
				...
				LinhaExtratoContaCorrenteBancoBrasil linha = (LinhaExtratoContaCorrenteBancoBrasil) modelTabNaoEncontrados.getValueAt(row, COLUMN_OBJECT);
	...
	public static void main(String[] args) {
		ProcessadorExtratoCCBancoBrasilGUI gui = new ProcessadorExtratoCCBancoBrasilGUI();
		gui.setVisible(true);
	}
	...
	public void mouseClicked(MouseEvent e) {
		if ( e.getSource() instanceof JTable ) {
			...
			if( row > -1 && col > -1 ) {
				LinhaExtratoContaCorrenteBancoBrasil linha  = (LinhaExtratoContaCorrenteBancoBrasil)  tab.getModel().getValueAt(row, COLUMN_OBJECT);
	...
	public void actionPerformed(ActionEvent e) {
		if ( e.getSource() instanceof JButton ) {
			...
			int row = this.tabNaoEncontrados.getSelectedRow();
			int col = this.tabNaoEncontrados.getSelectedColumn();
			
			if ( row > -1 && col > -1 ) {
				
				if ( StringUtils.equals("TRANSFERIR", btn.getActionCommand())) {
					this.transferirLancamentoBaseDados(row);
				} else
			...
			if ( e.getSource() instanceof JComboBox ) {	
				...
				contaFrame.setVisible(true);
				if ( contaFrame.getConta() != null ) {
					j.getModel().setSelectedItem(contaFrame.getConta());
				}
			} else {
	...
	private class Salvar extends SwingWorker<String, String> {
		private ProcessadorExtratoCCBancoBrasilGUI frame;
		private LinhaExtratoContaCorrenteBancoBrasil linha;
		...
		protected String doInBackground() throws Exception {
			if ( "Transferência".equalsIgnoreCase(tipo) ) {
				Transferencia transferencia = ProcessadorExtratoCCBancoBrasil.converterLinhaTransferencia(linha, conta, cc, tipo);
				transferencia.setAtivoTransferido(getContaCorrenteAlvoTransferencia(), 
						  getContaContabilDeposito(),Deposito.class,"Depósito Banco do Brasil");
						  businessDelegate.saveTransferencia(transferencia);
			} else
			if ( linha.isPassivo() ) {
				Passivo passivo = ProcessadorExtratoCCBancoBrasil.converterLinhaPassivo(linha, conta, cc, tipo);
				businessDelegate.savePassivo(passivo);
			} else
			if ( linha.isAtivo()) {
				Ativo ativo = ProcessadorExtratoCCBancoBrasil.converterLinhaAtivo(linha, conta, cc, tipo);
	...
	private void transferirLancamentoBaseDados(int row) {
		if (!(this.modelTabNaoEncontrados.getValueAt(row, COLUMN_CONTA_CONTABIL) instanceof String)) {
			String ident = (String)this.modelTabNaoEncontrados.getValueAt(row, COLUMN_COMBO_IDENT);
			if ( StringUtils.isNotEmpty(ident)) {
				LinhaExtratoContaCorrenteBancoBrasil linha  = (LinhaExtratoContaCorrenteBancoBrasil) this.modelTabNaoEncontrados.getValueAt(row, COLUMN_OBJECT);
				Conta                     conta  = (Conta)this.modelTabNaoEncontrados.getValueAt(row, COLUMN_CONTA_CONTABIL);
